...
export default function MarketDetailScreen() {
  ...
  useEffect(() => {
    ...
    marketApi
      ...
      .catch((error) => notifyError(error, "Không thể tải thông tin tài sản."))
      .finally(() => setLoading(false));
  }, [symbol]);

  const price = useMemo(() => {
    if (displaySymbol.includes("ETH")) return 3450.2;
    if (displaySymbol.includes("SOL")) return 142.0;
    return 67432.12;
  }, [displaySymbol]);
  ...
  return (
    <Screen padded={false} scroll={false}>
      <View style={[styles.page, { backgroundColor: palette.bg }]}>
        <MarketHeader palette={palette} pro={isProfessionalLayout} symbol={displaySymbol} />

        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <QuoteSummary coin={coin} palette={palette} price={price} pro={isProfessionalLayout} />
          ...
          <AboutSection palette={palette} pro={isProfessionalLayout} symbol={displaySymbol} />
        </ScrollView>

        <TradeActions palette={palette} pro={isProfessionalLayout} symbol={displaySymbol} />
      </View>
    </Screen>
  );
}
...
function normalizeSymbol(symbol?: string) {
  const raw = (symbol || "BTCUSDT").toUpperCase().replace(/[^A-Z]/g, "");
  if (raw.includes("BTC")) return "BTC/USDT";
  if (raw.includes("ETH")) return "ETH/USDT";
  if (raw.includes("SOL")) return "SOL/USDT";
  return raw.includes("USDT") ? raw.replace("USDT", "/USDT") : raw;
}
    ...
    <View style={[styles.header, { borderBottomColor: palette.line }]}>
      ...
      <View style={styles.headerTitleWrap}>
        <View style={styles.headerSymbolRow}>
          <Text style={[styles.headerSymbol, { color: palette.text }]}>{symbol}</Text>
          {pro ? <Ionicons color={palette.muted} name="chevron-down" size={14} /> : null}
        </View>
        <Text style={[styles.headerChange, { color: palette.success }]}>+2.45%</Text>
      </View>

      <View style={styles.headerActions}>
        {pro ? <Ionicons color={palette.text} name="search-outline" size={23} /> : null}
  ...
  return (
    <View style={[styles.quoteBlock, pro && styles.quoteBlockPro]}>
      <View style={styles.priceBlock}>
        <Text style={[styles.bigPrice, { color: palette.success }]}>{formatMoney(price.toFixed(decimals))}</Text>
...
function AboutSection({
  ...
  const assetName = symbol.includes("BTC") ? "Bitcoin" : symbol.split("/")[0];
  ...
  return (
    <View style={[styles.aboutCard, { backgroundColor: palette.card, borderColor: palette.line }]}>
      ...
      <View style={styles.aboutStats}>
        <AboutStat label="VỐN HOÁ" palette={palette} value="$1.32T" />
        <AboutStat label="LƯU THÔNG" palette={palette} value="19.7M BTC" />
      </View>
    </View>
  );
}
